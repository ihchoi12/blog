...
ConWeave requires no changes to RDMA hardware and applications, removing a major practical barrier to deployment, while improving 99-percentile flow completion times by up to 66.8\% over existing load balancing schemes.
...
In preliminary evaluation, Autokernel sustains around 100$\mu$s tail latency under dynamic workloads where a statically-tuned baseline incurs millisecond-scale queuing delays. 
