...
\subsection{Capybara: Dynamic Server Load Balancing with TCP Migration (APSys \yr{23}, SIGCOMM \yr{26})~\cite{capybara, mako}}
...
Building on this foundation, I am extending cross-layer co-design in two further directions: into the operating system itself, and into the broader AI infrastructure layer.

\subsection{Co-Designing Operating Systems with Machine Learning (Ongoing)}

The host (and particularly the OS that mediates every I/O) has emerged as a major performance bottleneck in modern datacenters. Sustaining low latency and high throughput under increasingly demanding workloads requires the OS to adapt dynamically to workload characteristics, hardware behavior, and runtime system conditions. Yet the tuning surface is vast: dozens of parameters span networking, scheduling, and memory subsystems with non-obvious interactions, and optimal settings shift at microsecond timescales, far beyond what fixed rules or human-tuned heuristics can navigate. Autokernel~\cite{autokernel} addresses this with the first ML-native dataplane OS architecture, treating machine learning as a first-class system component for continuous, workload-adaptive optimization. It exposes a uniform tuning interface across these subsystems, embeds observation points throughout the system to generate high-quality training data, and uses a hybrid inference strategy combining offline lookup tables for well-understood scenarios with lightweight ML models for unseen workloads. The entire tuning loop runs within the tight latency budgets of dataplane I/O processing. In preliminary evaluation, Autokernel sustains around 100$\mu$s tail latency under dynamic workloads where a statically-tuned baseline incurs millisecond-scale queuing delays. Full-system development is ongoing in close collaboration with Microsoft Research.

Effective ML-driven OS tuning requires precise understanding of where bottlenecks actually arise inside the host and which parameters meaningfully affect performance. In a complementary ongoing effort, I am conducting a systematic analysis of the intra-host network: the hardware data path that carries packets between the NIC and CPU/memory through the PCIe complex, IOMMU/IOTLB, and DDIO cache. Using detailed hardware telemetry, kernel-bypass profiling, and parameter sweeps, the analysis reveals that key dataplane parameters are strongly regime-dependent, with optimal settings differing by orders of magnitude across workload conditions (packet size, offered rate, core count). No static default is best across the operating range, directly motivating learned, runtime-adaptive policies that fuse live telemetry with workload characteristics.
